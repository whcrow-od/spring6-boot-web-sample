...
public class BodyModelResponse<T> extends ResponseEntity<BodyModel<T>> {
	...
	public BodyModelResponse(@Nonnull BodyModel<T> bodyModel) {
		super(Assert.notNull(bodyModel, "responseBody"), bodyModel.getStatusCode());
	}
	
	public BodyModelResponse(@Nonnull HttpStatusCode status, @Nullable T payload, @Nullable String details) {
